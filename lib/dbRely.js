...
export async function findUserById(db, userId, colc = "users") {
  return db
    .collection(colc)
...
export async function findUserByTokenAndDelete(
  ...
  return db
    .collection(colc)
    .findOneAndDelete({$and: [ {confirmationCode: token , expires: { $gt: Date.now() }}]})
...
export async function insertUserVerificationTokens(
  ...
  const { captcha,passwordConfirmation,password, createdAt,...infoUser } = req.body;
  const user = {
    ...infoUser,
...
export async function findAndUpdate(req,query,newdoc,colc="user") {
console.log(query);
 return req.db.collection(colc).findOneAndUpdate(
      query ,
    { $set: {...newdoc,expires:Date.now()+3600000}  },
