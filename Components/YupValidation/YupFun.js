...
import { parseDateString, dateFormaNow } from "../FormsUi/DatePicker/index";
...
//   }
export function YupString(minlength = 1, maxlength = 20) {
  //الكتابة بالعربية
  return Yup.string()
    .required(Messages.required)
...
export function numStr(length = 16) {
  //رقم سترينغ محدد طول الاحرف
  return Yup.string()
...
export function numStrMinMax(min = 1, max = 16) {
  //رقم سترينغ محدد ادنى واقصى الاحرف
  return Yup.string()
    .matches(/^[0-9]*$/, Messages.digits)
...
export function YupDate(isRequired = true, date = new Date()) {
  return Yup.date()
    .required(Messages.required)
    .typeError(Messages.date)
    .transform(parseDateString)
    ...
    .max(
      dateFormaNow(date),
      Messages.rangeDate(1958, dateFormaNow(date) || "واليوم")
    );
}
export function BasicStr(maxlength = 200) {
  //سلسلة عادية
  return Yup.string()
    .required(Messages.required)
    .max(maxlength, Messages.maxlength(maxlength));
